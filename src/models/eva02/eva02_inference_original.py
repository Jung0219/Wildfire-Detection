...
# ============ CONFIG ============
PARENT_DIR      = "/lab/projects/fire_smoke_awr/outputs/eva02/ABCDE_early_fire_removed"
# test set (ImageFolder expects subdirs per class)
DATA_DIR        = "/lab/projects/fire_smoke_awr/data/classification/training/ABCDE_early_fire_removed/val"
...
with torch.no_grad():
    for batch_idx, (imgs, labels) in enumerate(tqdm(test_loader, desc="Evaluating")):
        ...
        for idx, pred in zip(batch_indices, preds):
            ...
            results[os.path.basename(fname)] = CLASSES[pred]
...
print(f"[{TRANSFORM_MODE}] Saved per-image predictions (3-class) to {save_path}")
