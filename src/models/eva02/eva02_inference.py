"""Run EVA-02 classifier inference and save background/foreground labels.

Example:
    python -m src.models.eva02.eva02_inference_original
"""
...
# ============ CONFIG ============
MODEL_DIR = "/lab/projects/fire_smoke_awr/outputs/eva02/AD_phash3_early_smoke" # model 
# test set (ImageFolder expects subdirs per class)
DATA_DIR = "/lab/projects/fire_smoke_awr/data/classification/training/AD_phash3_early_smoke/test" # test image directory
SAVE_SUBDIR = os.path.basename(DATA_DIR)

# Map classifier outputs to labels (0 -> background, 1 -> foreground).
CLASS_ID_TO_NAME = {0: "background", 1: "foreground"}
...
MODEL_PATH = os.path.join(MODEL_DIR, "train", "weights", "best_loss.pt")
SAVE_DIR = os.path.join(MODEL_DIR, SAVE_SUBDIR)
...
with torch.no_grad():
    for batch_idx, (imgs, labels) in enumerate(tqdm(test_loader, desc="Evaluating")):
        ...
        for idx, pred in zip(batch_indices, preds):
            ...
            results[os.path.basename(fname)] = CLASS_ID_TO_NAME.get(int(pred), "unknown")
