"""Folder runner for two-stage pipeline (detector + classifier) without multires composites.

Edit `CONFIG` to point at your YAML before running:
    python src/full_pipeline/run/batch/two_stage_batch_raw.py
"""
...
def main() -> None:
    ...
    cfg = MRClassifierConfig(
        ...
        anchor_y_frac=float(cfg_dict.get("anchor_y_frac", 0.0)),
    ...
    image_size = cfg_dict.get("image_size", cfg_dict.get("detector_imgsz", 640))

    for img_path in tqdm(image_paths, desc="Running two-stage (raw)"):
