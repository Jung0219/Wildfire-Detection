...
# post processing
SIZE_THRESH   = float("inf")  # still global
...
if __name__ == "__main__":
    ...
    parser.add_argument("--conf-top", type=float, default=0.0,
                        help="Confidence threshold for filtering boxes from TOP window only")
    args = parser.parse_args()
    CONF_THRESH = args.conf_top

    # output dir depends on fixed crop size
    OUTPUT_DIR = os.path.join(PARENT_DIR, f"experiments/selective_fusion_conf/crop{INTERMEDIATE_SIZE}_ct{CONF_THRESH}")
    ...
    for img_name in tqdm(image_files, desc=f"Processing images @ crop{INTERMEDIATE_SIZE}, ct={CONF_THRESH}"):
    ...
    print(f"✅ Finished processing {len(image_files)} images @ crop{INTERMEDIATE_SIZE}, ct={CONF_THRESH}. Results saved to {OUTPUT_DIR}")
