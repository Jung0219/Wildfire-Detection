...
# post processing
CONF_THRESH   = 0.0           # fixed, no confidence filtering
NMS_IOU_THRESH = None         # final NMS IoU threshold, None for No NMS
SAVE_IMG = False
# ==========================================
...
if __name__ == "__main__":
    ...
    parser.add_argument("--size-top", type=float, default=float("inf"),
                        help="Maximum sqrt(area) for boxes in the TOP window")
    args = parser.parse_args()
    SIZE_THRESH = args.size_top

    OUTPUT_DIR = os.path.join(PARENT_DIR, f"experiments/selective_fusion_box_size/crop{INTERMEDIATE_SIZE}_st{SIZE_THRESH}")
    ...
    for img_name in tqdm(image_files, desc=f"Processing images @ crop{INTERMEDIATE_SIZE}, st={SIZE_THRESH}"):
        ...
        # apply size filtering ONLY on top window
        filtered_top = []
        for det in dets_top:
    ...
    print(f"✅ Finished processing {len(image_files)} images @ crop{INTERMEDIATE_SIZE}, st={SIZE_THRESH}. Results saved to {OUTPUT_DIR}")
