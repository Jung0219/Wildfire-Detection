...
# post processing defaults (overwritten by argparse)
DEFAULT_CONF_THRESH = 0.0
DEFAULT_SIZE_THRESH = float("inf")

NMS_IOU_THRESH = None   # final NMS IoU threshold, None = no NMS
SAVE_IMG = False
# ==========================================
...
if __name__ == "__main__":
    ...
    parser.add_argument("--conf-top", type=float, default=DEFAULT_CONF_THRESH,
                        help="Confidence threshold for filtering boxes from TOP window only")
    parser.add_argument("--size-top", type=float, default=DEFAULT_SIZE_THRESH,
                        help="Box size threshold (sqrt(area)) for filtering boxes from TOP window only")
    args = parser.parse_args()

    CONF_THRESH = args.conf_top
    SIZE_THRESH = args.size_top

    # output dir depends on crop size and thresholds
    OUTPUT_DIR = os.path.join(PARENT_DIR, f"experiments/selective_fusion_box_conf/crop{INTERMEDIATE_SIZE}_ct{CONF_THRESH}_st{SIZE_THRESH}")
    ...
    for img_name in tqdm(image_files, desc=f"Processing images @ crop{INTERMEDIATE_SIZE}, ct={CONF_THRESH}, st={SIZE_THRESH}"):
    ...
    print(f"✅ Finished processing {len(image_files)} images @ crop{INTERMEDIATE_SIZE}, ct={CONF_THRESH}, st={SIZE_THRESH}. Results saved to {OUTPUT_DIR}")
